data1<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/sigma64.csv",head=T)
data2<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/hop_growth.csv",head=T)
data3<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/new7_old69.csv",head=T)
data4<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/decreasing.csv",head=T)
...
ndata12<- data.frame(m0=c(data11$m0,data12$m0,data13$m0,data14$m0),fatalities=c(data11$fatalities,data12$fatalities,data13$fatalities,data14$fatalities),name=c(rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71),rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71),rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71),rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71)),case=c(rep("Lower inward protection rates",284),rep("Faster mask growth",284),rep("Similar protection rates of new and old masks",284),rep("Higher initial stock of masks",284)))

ndata22<- data.frame(m0=c(data21$m0,data22$m0,data23$m0,data24$m0),ic=c(data21$ic,data22$ic,data23$ic,data24$ic),name=c(rep("Agents without masks",71), rep("Agents with masks",71),rep("Agents without masks",71), rep("Agents with masks",71),rep("Agents without masks",71), rep("Agents with masks",71),rep("Agents without masks",71), rep("Agents with masks",71)),case=c(rep("Lower inward protection rates",142), rep("Faster mask growth",142), rep("Similar protection rates of new and old masks",142), rep("Higher initial stock of masks",142)))
...
index.cond=list(c(1,2,4,3)),#this provides the order of the panels
...
m<- 3.75
pdf("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/final_graphs/Robust_1.pdf",width =1.618*1.8*m, height = 3*m) 
