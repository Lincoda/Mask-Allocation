...
data2<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/hetero.csv",head=T)
data3<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/hetero_old0.27.csv",head=T)
...
ndata12<- data.frame(m0=c(data12$m0,data13$m0),fatalities=c(data12$fatalities,data13$fatalities),name=c(rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71),rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71)),case=c(rep("Heterogeneous agents",284),rep("Heterogeneous agents with lower mask protection",284)))

ndata22<- data.frame(m0=c(data22$m0,data23$m0),ic=c(data22$ic,data23$ic),name=c(rep("Agents without masks",71), rep("Agents with masks",71),rep("Agents without masks",71), rep("Agents with masks",71)),case=c(rep("Heterogeneous agents",142), rep("Heterogeneous agents with lower mask protection",142)))
...
tp1<- xyplot(fatalities*100000~m0|case, groups = name, data=ndata12, grid=F,layout=c(1,2),col=c("slategray4","slategray3","gray20","firebrick2"),
...
tp2<- xyplot(ic~m0|case, groups = name, data=ndata22, grid=F,layout=c(1,2),col=c("gray20","firebrick2"),
index.cond=list(c(2,1)),#this provides the order of the panels
...
m<- 3.75
pdf("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/final_graphs/extension_hetero.pdf",width =1.618*1.8*m, height = 1.75*m) 
