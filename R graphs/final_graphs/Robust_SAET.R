...
data3<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/decreasing.csv",head=T)
...
ndata12<- data.frame(m0=c(data12$m0,data13$m0),fatalities=c(data12$fatalities,data13$fatalities),name=c(rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71),rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71)),case=c(rep("Faster mask growth",284),rep("Higher initial stock of masks",284)))

ndata22<- data.frame(m0=c(data22$m0,data23$m0),ic=c(data22$ic,data23$ic),name=c(rep("Agents without masks",71), rep("Agents with masks",71),rep("Agents without masks",71), rep("Agents with masks",71)),case=c(rep("Faster mask growth",142), rep("Higher initial stock of masks",142)))
...
tp1<- xyplot(fatalities*100000~m0|case, groups = name, data=ndata12, grid=F,layout=c(1,2),col=c("slategray4","slategray3","black","firebrick2"),
index.cond=list(c(1,2)),#this provides the order of the panels
key = key.variety,pch=c(16,1,16,1),cex=0.75,
xlab="Coverage rate of new masks to the population at t=1", ylab="Mortality (100,000 population)",type="p",ylim=c(0.75*100000*min(ndata12$fatalities),1.1*100000*max(ndata12$fatalities)),xlim=c(0.08,0.82),
...
tp2<- xyplot(ic~m0|case, groups = name, data=ndata22, grid=F,layout=c(1,2),col=c("black","firebrick2"),
index.cond=list(c(1,2)),#this provides the order of the panels
...
m<- 3.75
pdf("/Users/yicheng/Dropbox/COVID-19-Mask/要上傳的程式碼/R graphs/final_graphs/Robust_SAET.pdf",width =1.618*1.8*m, height = 1.75*m) 
