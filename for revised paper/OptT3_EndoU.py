...
class nation_SIRD :
    ...
    def find_optimal(self,m0,m1,m2):
        ...
        P2Φ = m.Intermediate(dSΦΦ/SΦΦ)
        P2o = m.Intermediate((1-σo)*P2Φ)
        P2n = m.Intermediate((1-σn)*P2Φ)
        ...
        P3Φ = m.Intermediate(dSΦ2/SΦ2)
        P3o = m.Intermediate((1-σo)*P3Φ)
        P3n = m.Intermediate((1-σn)*P3Φ)
        ...
        m.Equation(   𝜋B *πB1*( (1-P2n)*(1-P3n) ) +   𝜋B *(1-πB1)*( (1-P2n)*(1-P3o) )+
                   (1-𝜋B)*πB0*( (1-P2Φ)*(1-P3n) ) +(1-𝜋B)*(1-πB0)*( (1-P2Φ)*(1-P3Φ) )>=
                      𝜋A *πA1*( (1-P2n)*(1-P3n) ) +   𝜋A *(1-πA1)*( (1-P2n)*(1-P3o) )+
                   (1-𝜋A)*πA0*( (1-P2Φ)*(1-P3n) ) +(1-𝜋A)*(1-πA0)*( (1-P2Φ)*(1-P3Φ) )) # for people who don't receive masks at t=0
        m.Equation(   𝜋A *𝜋A1*( (1-P2n)*(1-P3n) ) +   𝜋A *(1-𝜋A1)*( (1-P2n)*(1-P3o) )+
                   (1-𝜋A)*𝜋A0*( (1-P2o)*(1-P3n) ) +(1-𝜋A)*(1-𝜋A0)*( (1-P2o)*(1-P3Φ) )>=
                      𝜋B *𝜋B1*( (1-P2n)*(1-P3n) ) +   𝜋B *(1-𝜋B1)*( (1-P2n)*(1-P3o) )+
                   (1-𝜋B)*𝜋B0*( (1-P2o)*(1-P3n) ) +(1-𝜋B)*(1-𝜋B0)*( (1-P2o)*(1-P3Φ) )) # for people who receive masks at t=0
        ...
        return m.options.objfcnval,πA.value[0],πB.value[0],πA1.value[0],πB1.value[0],πA0.value[0],πB0.value[0]

    '''calculate the probability of reciving a mask in optimal * mechanism'''
    def find_optimal_star(self,m0,m1,m2):

        S0,I0 = self.S0, self.I0
        𝛽, 𝛾, α        = self.𝛽, self.𝛾, self.α
        σo, σn, δo, δn = self.σo, self.σn, self.δo, self.δn
        D0 = 𝛾*α*I0    # Compute the number of death in period 0
