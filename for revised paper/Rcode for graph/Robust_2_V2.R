data1<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/RED/Second_Review/data/All_graphs/data/new7_old69_vo.csv",head=T)
data2<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/RED/Second_Review/data/All_graphs/data/sigma64_vo.csv",head=T)
data3<- read.csv("/Users/yicheng/Dropbox/COVID-19-Mask/RED/Second_Review/data/All_graphs/data/rho05.csv",head=T)
...
ndata12<- data.frame(m0=c(data11$m0,data12$m0,data13$m0),fatalities=c(data11$fatalities,data12$fatalities,data13$fatalities),name=c(rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71),rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71),rep("Optimal",71),rep("GRBT",71),rep("SRA-I",71),rep("SRA-II",71)),case=c(rep("B: Dependent utility with similar mask protection rates",284),rep("A: Dependent utility with lower inward protection rates",284),rep("C: Lower discount rate",284)))

ndata22<- data.frame(m0=c(data21$m0,data22$m0,data23$m0),ic=c(data21$ic,data22$ic,data23$ic),name=c(rep("Agents without masks",71), rep("Agents with masks",71),rep("Agents without masks",71), rep("Agents with masks",71),rep("Agents without masks",71), rep("Agents with masks",71)),case=c(rep("B: Dependent utility with similar mask protection rates",142),rep("A: Dependent utility with lower inward protection rates",142), rep("C: Lower discount rate",142)))
...
m<- 3.75
pdf("/Users/yicheng/Dropbox/COVID-19-Mask/RED/Second_Review/data/All_graphs/graph/Robust_2.pdf",width =1.618*1.85*m, height = 2.5*m) 
